...
const handlers = {
  index(data, callback) {
    if (data.method === 'get') {
      const templateData = {
        ...
        'head.description': 'We offer uptime monitoring services',
        'body.class': 'home'
      };

      helpers.getTemplate('index', templateData, (err, htmlString) => {
        if (!err && htmlString) {
          helpers.addUniversalTemplates(htmlString, templateData, (err, combinedStrings) => {
            if (!err && combinedStrings) {
              callback(200, combinedStrings, 'html');
            } else {
  ...
  accountCreate(data, callback) {
    if (data.method === 'get') {
      ...
      helpers.getTemplate('accountCreate', templateData, (err, htmlString) => {
        if (!err && htmlString) {
          helpers.addUniversalTemplates(htmlString, templateData, (err, combinedStrings) => {
            if (!err && combinedStrings) {
              callback(200, combinedStrings, 'html');
  ...
  sessionCreate(data, callback) {
    if (data.method === 'get') {
      ...
      helpers.getTemplate('sessionCreate', templateData, (err, htmlString) => {
        if (!err && htmlString) {
          helpers.addUniversalTemplates(htmlString, templateData, (err, combinedStrings) => {
            if (!err && combinedStrings) {
              callback(200, combinedStrings, 'html');
  ...
  sessionDeleted(data, callback) {
    if (data.method === 'get') {
      ...
      helpers.getTemplate('sessionDeleted', templateData, (err, htmlString) => {
        if (!err && htmlString) {
          helpers.addUniversalTemplates(htmlString, templateData, (err, combinedStrings) => {
            if (!err && combinedStrings) {
              callback(200, combinedStrings, 'html');
  ...
  accountEdit(data, callback) {
    if (data.method === 'get') {
      ...
      helpers.getTemplate('accountEdit', templateData, (err, htmlString) => {
        if (!err && htmlString) {
          helpers.addUniversalTemplates(htmlString, templateData, (err, combinedStrings) => {
            if (!err && combinedStrings) {
              callback(200, combinedStrings, 'html');
  ...
  accountDeleted(data, callback) {
    if (data.method === 'get') {
      ...
      helpers.getTemplate('accountDeleted', templateData, (err, htmlString) => {
        if (!err && htmlString) {
          helpers.addUniversalTemplates(htmlString, templateData, (err, combinedStrings) => {
            if (!err && combinedStrings) {
              callback(200, combinedStrings, 'html');
  ...
  checksCreate(data, callback) {
    if (data.method === 'get') {
      ...
      helpers.getTemplate('checksCreate', templateData, (err, htmlString) => {
        if (!err && htmlString) {
          helpers.addUniversalTemplates(htmlString, templateData, (err, combinedStrings) => {
            if (!err && combinedStrings) {
              callback(200, combinedStrings, 'html');
  ...
  checksList(data, callback) {
    if (data.method === 'get') {
      const templateData = {
        ...
        'body.class': 'checks-list'
      };

      helpers.getTemplate('checksList', templateData, (err, htmlString) => {
        if (!err && htmlString) {
          helpers.addUniversalTemplates(htmlString, templateData, (err, combinedStrings) => {
            if (!err && combinedStrings) {
              callback(200, combinedStrings, 'html');
  ...
  checksEdit(data, callback) {
    if (data.method === 'get') {
      const templateData = {
        ...
        'body.class': 'checks-edit'
      };

      helpers.getTemplate('checksEdit', templateData, (err, htmlString) => {
        if (!err && htmlString) {
          helpers.addUniversalTemplates(htmlString, templateData, (err, combinedStrings) => {
            if (!err && combinedStrings) {
              callback(200, combinedStrings, 'html');
    ...
    if (data.method === 'get') {
      ...
      if (trimmedAssetName.length > 0) {
        helpers.getStaticAsset(trimmedAssetName, (err, data) => {
          if (!err && data) {
            // Default content type if none of others are chosen
  ...
  _users: {
    ...
    post(data, callback) {
      ...
      const lastName =
        typeof data.payload.lastName === 'string' && data.payload.lastName.trim().length > 0
          ...
          : false;
      const phone =
        typeof data.payload.phone === 'string' && data.payload.phone.trim().length > 7
        ...
        _data.read('users', phone, (err, userData) => {
          if (err) {
            ...
            if (hashedPassword) {
              ...
              callback(500, {
                Error: "Error trying to hash the user's password."
              });
            }
          } else {
            callback(400, {
    ...
    get(data, callback) {
      const phone =
        typeof data.queryStringObject.phone === 'string' && data.queryStringObject.phone.trim().length > 7
    ...
    put(data, callback) {
      ...
      if (phone) {
        if (firstName || lastName || password) {
          ...
          callback(400, {
            Error: 'Missing at least one optional field for update.'
          });
        }
      } else {
        callback(400, { Error: 'Missing required fields.' });
